...
export class ControlsRotatePlugin extends Plugin.BasePlugin {
  ...
  init() {
    ...
    // 旋转时，实时更新旋转控制图标
    this._canvas.on('object:rotating', (event) => {
      const body = this._canvas.lowerCanvasEl.nextSibling as HTMLElement
      const angle = this._canvas.getActiveObject()?.angle?.toFixed(2)
      if (angle === undefined)
        return
