...
function initRuler(canvas: fabric.Canvas, options?: RulerOptions) {
  ...
  // 辅助线移动到画板外删除
  let workspace: fabric.Object | undefined

  /**
   * 获取workspace
   */
  const getWorkspace = () => {
    workspace = canvas.getObjects().find(item => item.name === 'workspace')
  }
